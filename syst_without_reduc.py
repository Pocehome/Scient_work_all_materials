...
from system_dynamic import xy_dyn, num_integration, draw_graph, draw_start_end
...
def full_syst_stability_determination(N, mu, eps1, alp1, eps2, alp2):
...
def draw_full_syst(arr_sol, arr_t, T, start=True, end=True, T_inter=200, relative0=True):
    ...
    draw_start_end(np.mod(arr_thetas, 2*np.pi) - np.pi, arr_t, r'$\theta_k$', T,
                    draw_start=True, draw_end=True, T_inter=T_inter)
...
def find_init_state(arr_sol, arr_t):
    for i in range(len(arr_t)):
        if arr_t[i] < 100: continue
    
        # if abs(arr_sol[2][i] - 2*np.pi) < 1e-3 and abs(arr_sol[2][i] - arr_sol[0][i]) < 1e-3:
        #     print(i)
        #     break
        
        if (abs(np.mod(arr_sol[0][i] + np.pi, 2*np.pi) - np.pi) < 1e-2 and
            abs(np.mod(arr_sol[2][i] + np.pi, 2*np.pi) - np.pi) < 1e-2):
            print(i)
...
if __name__ == '__main__':
    ...
    draw_full_syst(arr_sol, arr_t, arr_t[-1], T_inter=100, relative0=True)
    
    # find_init_state(arr_sol, arr_t)
