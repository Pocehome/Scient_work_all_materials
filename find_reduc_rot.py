...
def get_xy_from_vec_i(N, mu, eps1, alp1, eps2, alp2):
...
def find_next_vec(vec_i, fg, rhs, calc_xy):
    xy = calc_xy(vec_i)
    matrix = np.column_stack((FG_dxp(vec_i, fg, rhs),
                              FG_dy(vec_i, fg, rhs),
                              FG_dyp(vec_i, fg, rhs),
                              xy))
...
def find_initial_vec(vec_0, rhs, calc_xy, f_stab_det, do_stab_det=False):
    vec_i = vec_0
    find_flag = False
    is_stable = None
    eigv = None
...
def full_finding_func(params, vec_0, do_stable_det=False, printing=False):
    ...
    # Newton's method
    initial_vec, find_flag, isStable, eigv = find_initial_vec(vec_0, rhs, calc_xy, f_stab_det, do_stab_det=do_stable_det)
    if printing:
        if do_stable_det:
